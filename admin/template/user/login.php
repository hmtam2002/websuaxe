    ...
    if (!empty($userQuery))
    {
        ...
        if (password_verify($password, $passwordHash))
        {
            ...
            if ($insertStatus)
            {
                ...
                $func->redirect('?com=user&act=update');
            } else
            {
...
<div class="position-fixed left-0 top-0 w-100 h-100">
    <p class="text-center bg-dark py-2 mb-0">
        <a class="text-decoration-none text-white" href="../" target="_blank" style="font-size: 12px;">
            <i class="fa fa-solic fa-reply"></i>
            <span>Xem website</span>
        </a>
    </p>
    <div class="w-100 h-100 bg-light d-flex justify-content-center align-items-center pb-5">
        <div class="border p-4 rounded-3 bg-white shadow-lg mb-5" style="max-width: 400px;">
            <p class="text-center mb-3 fs-4">Đăng nhập Hệ thống</p>
            <?php if (!empty($smg))
            ...
            <form method="post">
                ...
                <button type="submit" name="login" class="btn btn-secondary">Đăng nhập</button>
            </form>
        </div>
    </div>
</div>
