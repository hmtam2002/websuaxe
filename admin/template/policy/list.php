<?php
$listNews = $db->getRaw("SELECT * FROM new WHERE type = 'policy'");
if (empty($listNews))
{
...
<main id="content" class="col-md-9 ms-auto col-lg-10 px-md-4 py-4">
    <a href="?com=policy&act=add" class="btn btn-success mb-4">Thêm chính sách</a>
    <?php if (!empty($smg))
    {
    ...
    <table class="table <?= empty($listNews) ? 'd-none' : '' ?>">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="?com=policy&act=edit&id=<?= $item['id'] ?>" class="text-decoration-none text-dark">
                        <?= $item['title'] ?>
                    </a>
                </td>
                <td>
                    <a href="?com=policy&act=edit&id=<?= $item['id'] ?>" class=" btn btn-success btn-sm"
                        class="btn btn-warning btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                </td>
                <td>
                    <a href="?com=policy&act=delete&id=<?= $item['id'] ?>" data_id=" <?= $item['id'] ?>"
